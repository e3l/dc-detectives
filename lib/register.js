...
async function getSummaryOfOrder(orderNumber) {
    try {
        ...
        // Clean the text of U.S.C stuff, change semicolons to periods so the summarizer can count sentences.
        fullText = fullText.replace(/\s?\([^\(]*?U\.S\.(.*?\))(\(.*?\))?\)?/g, '').replace(/\;/g, '.');
        ...
        let summary = await summarizer.getSummaryByRank().then((summary_object) => {
            return summary_object.summary.replace(/\./g, '. ');
        });

        return { summary: summary, fullText: fullText };
    } catch (err) {
        console.log(err);
        return null;
    }
...
function elementText(element) {
    if (element.type == 'element') {
        ...
        // let halfclean = element.text.replace(";", ".").replace(/^\./, '');

        // console.log('half: ' + halfclean);

        // let fullclean = halfclean.replace(/\s?\([^\(]*?U\.S\.(.*?\))(\(.*?\))?\s*\)?/g, '');

        // console.log('full: '+ fullclean);

        let clean = element.text.replace(/^\./, '');
        return clean;
    }
}
